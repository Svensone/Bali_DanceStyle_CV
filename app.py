...
#### Classifing images of famous balinese dances styles: *Kecak* , *Barong*  &  *Legong*
**Dataset**: 500 Images scraped from Web, cleaned and preprocessed.
**Accuracy** on ResNet34 Architectur: 80% 
""")
...
def prediction(img, display_img):
    ...
    with st.spinner('Wait a second .....'):
        time.sleep(3)
    # Option 1. setup model & load weights
    ...
    # Option 2:
    learn = load_learner('v2-resnet34.pkl')

    # Prediction on Image
    predict_class = learn.predict(img)[0]
    predict_proba = learn.predict(img)[2]
    # print(predict_class, predict_proba)

    # Display results
    if str(predict_class) == 'legong':
        st.success(f'This is a Scene of the famous Legong Kraton Dance. Probability of Prediction is {round(float(predict_proba[2])*100, 2)} % ')
        link = 'Find out more [Wikipedia](https://en.wikipedia.org/wiki/Legong)'
        st.markdown(link, unsafe_allow_html=True)
    elif str(predict_class) == "barong":
        st.success(f'Barong Dance. Barong is, together with Sanghyang, considered to be an ancient native Balinese Dance. Probability of Prediction is {round(float(predict_proba[0])*100, 2)} %')
        link = '[Barong Wikipedia](https://en.wikipedia.org/wiki/Barong_(mythology)#Barong_dance)'
        st.markdown(link, unsafe_allow_html=True)
    else:
        st.success(f'This is a Scene of the Kecak Dance, created by german artist Walter Spies in 1930s. Probability of Prediction is {round((float(predict_proba[1])*100), 2)} % ')
